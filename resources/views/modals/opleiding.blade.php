<div class="modal" id="modal-opleiding">
    <div class="modal-content">
            <div class="row">
                <div class="col-2-gt-30 modal-header-title">
                    <h1>Opleiding Toevoegen</h1>
                </div>
                <div class="col-2-gt-30 modal-header">
                    <div class="modal-close float-right" id="modal-opleiding-close">
                        <i class="fa fa-close" id="navigation-close"></i>
                    </div>
                </div>
            </div>
            <div class="row">
                    <div class="form-group">
                        <label for="opleiding-naam">Naam</label>
                        <input type="text" class="textbox" id="opleiding-naam" placeholder="Naam van de opleiding">
                    </div>
                    <div class="form-group">
                        <label for="opleiding-beschrijving">Beschrijving</label>
                        <textarea cols="30" rows="7" class="textbox" id="opleiding-beschrijving" placeholder="Typ een beschrijving van de opleiding hier"></textarea>
                    </div>
                    <div class="form-group">
                        <label for="opleiding-link">Link</label>
                        <input type="text" class="textbox" id="opleiding-link" placeholder="Typ een link naar extra info over de opleiding">
                    </div>
            </div>
            <div class="row">
                <div class="col-2-gt-30">
                    <button class="button--delete button--block hidden" id="opleiding-verwijderen">Opleiding Verwijderen</button>
                </div>
                <div class="col-2-gt-30">
                    <button class="button--primary button--block" id="opleiding-toevoegen">Opleiding Toevoegen</button>
                </div>
            </div>
    </div>
</div>